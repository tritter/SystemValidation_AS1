...
#define DIR_STATUS_FIX
#define ROOT_CLOSE_CREATE_FIX
#define DIR_TREE_OPEN_FIX
...
int main(){
  ...
  init_fs();
    ...
    //determine file and directory number non deterministically
    int file = 1;//nondet_int();
    __CPROVER_assume(0 <= file && file < MAX_FILES);
    int dir = 1;//nondet_int();
...
int close_dir(int id){
    ...
    //this part does nothing with dir status, checked seperately, so disable to reduce state-space
    #ifndef ROOT_CLOSE_CREATE
    #ifndef DIR_TREE_OPEN
      for(i=0;i<MAX_FILES;i++){
          if (file_parent[i]==id){
                //close_file(i);
                //TESTED CLOSE FILE SEPERATELY, SO INSERTED THIS HERE TO REDUCE STATE-SPACE
